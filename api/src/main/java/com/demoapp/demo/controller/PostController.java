...
public class PostController {
  ...
  public ResponseEntity<?> searchPosts(
    ...
    try {
      Map<String, Object> posts = postService.searchPosts(query, tag, limit, skip, userId);
      return ResponseEntity.ok(posts);
    } catch (Exception e) {
      return ResponseEntity
          .status(500)
  ...
  public ResponseEntity<?> getTags() {
    try {
      Map<String, Object> tags = postService.getTags();
      return ResponseEntity.ok(tags);
    } catch (Exception e) {
      return ResponseEntity
          .status(500)
          .body(new ErrorResponse("Erro ao buscar tags: " + e.getMessage(), 500));
    }
  }
